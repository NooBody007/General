...
def register():
    ...
    if request.method == "POST" and form.validate():
        ...
        cursor.close()
        flash("Kayıt Olma İşlemi Başarıyla Gerçekleştirildi", category="success")
        return redirect(url_for("login"))
    else:
...
def login():
    ...
    if request.method == "POST":
        ...
        if result > 0:
            ...
            flash("Böyle bir kullanıcı bulunmamakta",category = "danger")
            return redirect(url_for("login"))
...
def update(id):
    if request.method =="GET":
        ...
        if result == 0:
            flash("Bu makale bulunmuyor ya da bu işleme yetkiniz yok","danger")
            return redirect(url_for("index"))
        else:
            article = cursor.fetchone()
            form = ArticleForm()
